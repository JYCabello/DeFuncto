    ...
    public readonly struct Du7<T1, T2, T3, T4, T5, T6, T7> : IEquatable<Du7<T1, T2, T3, T4, T5, T6, T7>>
    {
        ...
        public Unit Iter(Action<T1> ont1, Action<T2> ont2, Action<T3> ont3, Action<T4> ont4, Action<T5> ont5, Action<T6> ont6, Action<T7> ont7) =>
            Match(
                ...
                t7 => { ont7(t7); return unit; }
            );

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Unit Iter(Action<T1> fSome) =>
            Iter(fSome, _ => { }, _ => { }, _ => { }, _ => { }, _ => { }, _ => { });

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Unit Iter(Action<T2> fSome) =>
            Iter(_ => { }, fSome, _ => { }, _ => { }, _ => { }, _ => { }, _ => { });

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Unit Iter(Action<T3> fSome) =>
            Iter(_ => { }, _ => { }, fSome, _ => { }, _ => { }, _ => { }, _ => { });
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Unit Iter(Action<T4> fSome) =>
            Iter(_ => { }, _ => { }, _ => { }, fSome, _ => { }, _ => { }, _ => { });
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Unit Iter(Action<T5> fSome) =>
            Iter(_ => { }, _ => { }, _ => { }, _ => { }, fSome, _ => { }, _ => { });
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Unit Iter(Action<T6> fSome) =>
            Iter(_ => { }, _ => { }, _ => { }, _ => { }, _ => { }, fSome, _ => { });
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Unit Iter(Action<T7> fSome) =>
            Iter(_ => { }, _ => { }, _ => { }, _ => { }, _ => { }, _ => { }, fSome);
        ...
        [Pure]
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Unit Iter(Func<T7, Unit> ont4) =>
            Iter(t7 => { ont4(t7); });
    }
}
